...
export const ResumoCompra = (props) => {
  ...
  if (pratos.length === 0) {
    ...
    fechamentoElement = pratos.map((prato) => {
      totalPedido +=
        parseFloat(prato.preco_base) + parseFloat(prato.preco_items);
      return (
        <>
          <Card
            title={prato.nome_prato}
            subTitle={"R$ " + parseFloat(prato.preco_base).toFixed(2)}
          >
            <b>Adicionais</b>
            <ul>
              {prato.itens_escolhidos.map((item) => {
                return (
                  <li key={item.nome_item}>
                    {item.nome_item} - {item.preco_item}
                  </li>
                );
              })}
            </ul>
            <hr></hr>
            <b>Endereço:</b> {props.pratos.user.address}
...
const mapDispatchToProps = (dispatch) => {
  return {
    onLimparCarrinho: () => {
      dispatch(actions.limparCarrinho());
    },
    onFinalizarPedido: () => {
      dispatch(actions.fecharCarrinho());
    },
  };
};
